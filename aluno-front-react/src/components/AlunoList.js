...
function AlunoList() {
  ...
  const getRowClass = (ira) => {
    ...
    return parseFloat(ira) >= parseFloat(7) ? "table-primary" : "table-danger";
  };
    ...
    <div>
      ...
      <div className="mb-3">
        <Button variant="info" onClick={toggleColorirLinhas} className="mt-2">
          {colorirLinhas ? "Remover Cores" : "Pintar por Média"}
        </Button>
      </div>
      <Table bordered hover className={colorirLinhas ? "" : "table-striped"}>
        ...
        <tbody>
          ...
          <tr className="table-dark">
            <td colSpan="2">
              <strong>Média IRA</strong>
            </td>
            <td colSpan="2">
              <strong>{mediaIRA}</strong>
            </td>
          </tr>
        </tbody>
      </Table>
